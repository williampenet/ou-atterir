-- ==========================================================
-- Ou Atterir — DVF price filter support
-- Adds target_prix_m2_max parameter to search/count functions
-- ==========================================================

BEGIN;

-- ==========================================================
-- 0. Materialized view: latest year price per commune
-- ==========================================================

CREATE MATERIALIZED VIEW IF NOT EXISTS commune_prix_m2_latest AS
SELECT
  code_insee,
  MIN(prix_m2_median) AS prix_m2_min
FROM commune_dvf_stats
WHERE year = (SELECT MAX(year) FROM commune_dvf_stats)
  AND prix_m2_median IS NOT NULL
GROUP BY code_insee;

CREATE UNIQUE INDEX IF NOT EXISTS idx_prix_m2_latest_insee ON commune_prix_m2_latest(code_insee);
CREATE INDEX IF NOT EXISTS idx_prix_m2_latest_prix ON commune_prix_m2_latest(prix_m2_min);

-- ==========================================================
-- 1. Replace search_communes with prix_m2 filter
-- ==========================================================

DROP FUNCTION IF EXISTS search_communes(text, text, text, text[], text[], text, text[], int, int);
...
-- ==========================================================
-- 2. Replace count_communes with prix_m2 filter
-- ==========================================================

DROP FUNCTION IF EXISTS count_communes(text, text, text, text[], text[], text, text[]);
